...
@dataclass(slots=True)
class SystemBreak:
    """A system (line) break in the source PDF layout.

    measure_number: The 1-indexed measure where a new system starts.
    page_number: 0-indexed page number in the source PDF.
    new_page: Whether this system break is also a page break.
    """
...
@dataclass(slots=True)
class LayoutInfo:
    """Layout information extracted from a source PDF."""

    system_breaks: list[SystemBreak]
    total_measures_source: int
