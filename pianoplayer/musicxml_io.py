...
from pianoplayer.models import INote, LayoutInfo, SystemBreak, keypos
...
def inject_layout_breaks(score_info: ScoreInfo, layout: LayoutInfo) -> bool:
    """Inject MusicXML `<print>` elements to preserve source PDF layout.

    Validates that the MusicXML measure count matches the PDF analysis
    within tolerance, then injects `<print>` elements at the correct
    measures in ALL parts.

    Args:
        score_info: Parsed MusicXML score info.
        layout: Layout information from PDF analysis.

    Returns:
        True if layout breaks were successfully injected, False if
        tolerance validation failed.
    """
    if not layout.system_breaks:
        logger.debug("No system breaks to inject")
        return False

    # Count measures from MusicXML.
    xml_total_measures = _count_measures_xml(score_info)
    pdf_total_measures = layout.total_measures_source

    # Tolerance: max(3, total_measures * 0.05).
    tolerance = max(3, int(pdf_total_measures * 0.05))

    if abs(xml_total_measures - pdf_total_measures) > tolerance:
        logger.warning(
            "Measure count mismatch: MusicXML has %d measures, PDF analysis "
            "found %d (tolerance: %d). Skipping layout preservation; MuseScore "
            "will use auto-reflow.",
            xml_total_measures,
            pdf_total_measures,
            tolerance,
        )
    ...
    # Inject into ALL parts.
    root = score_info.tree.getroot()
    if root is None:
        logger.warning("MusicXML tree has no root element; skipping layout injection")
        return False
    for part in score_info.parts:
        _inject_prints_into_part(root, part, layout)

    logger.info(
        "Injected %d layout break(s) into %d part(s) for %d-measure score",
        len(layout.system_breaks),
...
def _count_measures_xml(score_info: ScoreInfo) -> int:
    """Count total measures across all parts in a ScoreInfo object."""
    max_measures = 0
    root = score_info.tree.getroot()
    if root is None:
        return 0
    for part in score_info.parts:
        for p_el in root.findall("part"):
            if p_el.attrib.get("id", "") == part.part_id:
                measures = list(p_el.findall("measure"))
                max_measures = max(max_measures, len(measures))
                break
    return max_measures

def count_measures(musicxml_path: str) -> int:
    """Count total measures in a MusicXML file across all parts.

    Returns the maximum measure count found in any single part.
    """
    tree = ET.parse(musicxml_path)
    root = tree.getroot()
    max_measures = 0
    for part_el in root.findall("part"):
        measures = list(part_el.findall("measure"))
        max_measures = max(max_measures, len(measures))
    return max_measures


def _inject_prints_into_part(
    root: ET.Element, part: PartInfo, layout: LayoutInfo
) -> None:
    """Inject `<print>` elements into a single part at correct measures.

    Clears existing `<print>` elements before injection to avoid duplicates.
    """
    part_el = None
    for p_el in root.findall("part"):
        if p_el.attrib.get("id", "") == part.part_id:
    ...
    if part_el is None:
        logger.warning("Could not find part element for ID: %s", part.part_id)
        return

    # Clear existing <print> elements.
    for existing_print in list(part_el.findall("print")):
        part_el.remove(existing_print)

    # Build a map from measure number to SystemBreak for O(1) lookup.
    break_map: dict[int, SystemBreak] = {}
    for sb in layout.system_breaks:
        break_map[sb.measure_number] = sb

    # Find measures and inject <print> before each target measure.
    for measure_el in part_el.findall("measure"):
        raw_measure = (measure_el.attrib.get("number", "0") or "0").strip()
        try:
            measure_no = int(raw_measure)
        ...
        if measure_no in break_map:
            sb = break_map[measure_no]
            ...
            # Insert before the measure element in the part.
            idx = list(part_el).index(measure_el)
            part_el.insert(idx, print_el)
            logger.debug(
                "Injected <print> at measure %d (page %d, new_page=%s)",
                measure_no,
                sb.page_number,
                sb.new_page,
            )
